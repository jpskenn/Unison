...
- MIDI機能とスピーカーを使用するAudio機能は、同時に有効化できません。
    ...
    ファームウェアサイズが大きくなりすぎるため、書き込むことができません。
...
| キーキャップ<br/>挟ピッチに対応する、16mm＋α以下のもの | 同上 |
| ケース<br/>GH60型 | 1 |
| USBケーブル <br/>Type-C | 1 |
...
デバイスマネージャーの`キーボード`に表示されている`HID キーボードデバイス`を開き、`詳細`の`プロパティ`で`ハードウェア ID`を表示します。
...
[jpskenn/qmk_firmware: develop_Unisonブランチ](https://github.com/jpskenn/qmk_firmware/tree/develop_Unison/)をダウンロードし、`keyboards`ディレクトリ配下の`unison`ディクトリを、構築したQMKの`keyboards`ディレクトリ配下へ配置します。  
...
ファームウェアに定義されているLEDグループの組み合わせは、次のとおりです。  
...
- [取り付けるLEDに応じたジャンパ](#取り付けるLEDに応じたジャンパ)がおこなわれていない
    → 正しくジャンパする。
- [ファームウェアに定義されたLEDグループの組み合わせ](#ファームウェアに定義されたLEDグループの組み合わせ)と取り付けたLEDの個数、場所が異なっている
...
（個人的な感想として、TOFUケースと組み合わせたときは8mmのスペーサーは低すぎる気がします。）
